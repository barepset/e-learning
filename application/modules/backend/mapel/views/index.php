           ...
           ?>
			...
			<tbody>
				<tr>
					...
					<td><?php echo $no; ?></td>
					<td class="sorting_1"><?php echo $obj['mapel'] ?></td>
          <td>
          	<a href="<?=base_url(); ?>backend/mapel/ubah/<?php echo $obj['id_mapel']; ?>"class='btn btn-xs btn-flat btn-info'>Ubah</a>
            <a href="javascript:if(confirm('Apakah Anda Yakin Menghapus Data ini  ?')){document.location='<?php echo base_url(); ?>backend/foto/hapus/<?php echo $obj['id_mapel']; ?>';}" class='btn btn-flat btn-xs btn-danger'>Hapus</a>
          </td>
				</tr>
			</tbody>
				<?php
